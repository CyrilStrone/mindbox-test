...
import { TodoNewProps, TodoValueProps } from '../organelles/todo'

export const addNewItem = (newText: string, value: TodoValueProps[] | null, setValue: React.Dispatch<React.SetStateAction<TodoValueProps[] | null>>, setNewText: React.Dispatch<SetStateAction<string>>) => {
  if (!newText) return
  const newItem = {
    text: newText,
    isCheck: false,
...
export function TodoNew(props: TodoNewProps) {
  ...
  return (
    <div className={props.isShow ? "TodoNew--show TodoNew" : "TodoNew--hide TodoNew"}>
      <div data-testid="TodoNew__Icon" className='TodoNew__Icon' onClick={changeShow}>
        {props.iconTodoNew ? props.iconTodoNew : ICON_NEW}
      </div>
      <form onSubmit={e => { e.preventDefault(); addNewItem(newText, props.value, props.setValue, setNewText) }} className='TodoNew__Form'>
        <input
          value={newText}
